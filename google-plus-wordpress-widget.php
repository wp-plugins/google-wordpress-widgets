...
function gplus_style(){
	echo '<style type="text/css">.widget_google_plus{border:none;background-color:#F2F2F2;-moz-border-radius:5px;-khtml-border-radius:5px;-webkit-border-radius:5px;border-radius:5px;padding:1px;}ul.plus li{background-color:#fff;margin:1px;padding:6px;border:none;}.widget_google_plus .widget-title,.widget_google_plus .widgettitle{background-color:#F2F2F2;height:48px;padding:9px;margin-bottom:1px;border:none;}img.plus{border:none;float:left;margin-right:10px;-moz-border-radius:5px;-khtml-border-radius:5px;-webkit-border-radius:5px;border-radius:5px;-webkit-box-shadow:2px 2px 4px rgba(0,0, 0, 0.3);-moz-box-shadow:2px 2px 4px rgba(0,0, 0, 0.3);box-shadow:2px 2px 4px rgba(0,0, 0, 0.3);}img.plus:hover{margin-right:6px;-moz-border-radius:40px;-khtml-border-radius:40px;-webkit-border-radius:40px;border-radius:40px;border:solid 2px #00CD00;}span.note {display:block;color:#ccc;}span.credit{text-align:right;margin-top:5px;margin-right:5px;background-color:#F2F2F2;display:block;}</style>';
}
...
class Google_Plus_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !$gson = wp_cache_get( 'widget-gplus-' . $this->number , 'widget' ) ) {
			...
			$apikey = 'ABQIAAAAUHFseoe76iE2hWessar5dhTT1cjM_1wYB7ps6k85Lgo5CDGgVhTBGcZazEC4vVDYUbZHnEb00o58hA';
			...
			if( $gson || $gson->data || $gson->data->results ) {
				...
				$g_photo = $gson->data->thumbnailUrl .'?sz=40';
		...
		if ( $g_title ) {
			...
			if ( $author->get_name() ) $title = $author->get_name();
				echo "{$before_widget}{$before_title}<a title='Add " . esc_html("+".$title) . " to circles' href='" . esc_url( "https://plus.google.com/{$account}" ) . "'><img title='" . esc_html("+".$title) . "' class='plus' src='http://t3.gstatic.com/images?q=tbn:ANd9GcSmrt8RmWMZXBYCmCz8hmgbbE36WtiCo5coU2BhOPTlxWAdeetZJtY65g' alt='google' height='40' width='40' /> " . esc_html("+".$title) . "</a>{$after_title}";
		}
