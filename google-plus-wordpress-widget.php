...
class Google_Plus_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $g_title ) {
			echo "{$before_widget}{$before_title}<a rel='author' target='_blank' title='Add " . esc_html("+".$g_title) . " to circles' href='" . esc_url( "https://plus.google.com/{$account}" ) . "'><img title='" . esc_html("+".$g_title) . "' class='plus' src='". $g_photo ."' alt='google' height='40' width='40' /> " . esc_html("+".$g_title) . "</a>";

			if ( !$hidework ) {
				echo "<span class='note'>". $g_slogan ."</span>";
			} else {
				echo "<span class='note'><a rel='author' target='_blank' class='circles' title='Add " . esc_html("+".$g_title) . " to circles' href='" . esc_url( "https://plus.google.com/{$account}" ) . "'>Add To Circles</a></span>";
			}
			...
			if ( $author->get_name() ) $title = $author->get_name();
				echo "{$before_widget}{$before_title}<a rel='author' target='_blank' title='Add " . esc_html("+".$title) . " to circles' href='" . esc_url( "https://plus.google.com/{$account}" ) . "'><img title='" . esc_html("+".$title) . "' class='plus' src='http://t2.gstatic.com/images?q=tbn:ANd9GcQ9TgoKZN1g2u87Oey4aZSahAT8zHUp9eP7FHtED-eiBcIDX3QQrU8d' alt='google' height='40' width='40' /> " . esc_html("+".$title) . "</a>{$after_title}";
		}
		...
		echo "</div><!-- Google+ Plus Wordpress Widget by Patrick http://patrick.bloggles.info/ -->\n";
		$rss->__destruct();
		unset($rss);
