...
?>
<style type="text/css">.widget_google_plus a{color:#000;}
.author-box{padding:10px;}
.widget_google_plus{-moz-box-shadow:inset 0 0 15px #ccc;-webkit-box-shadow:inset 0 0 15px #ccc;box-shadow:inset 0 0 15px #ccc;font-family:arial, hevetica;border:solid 1px #ccc;background-color:#fff;padding:0; text-align:center;margin-bottom:15px;}
a.plus_person, a.plus_person:hover, a.plus_person:visited{color:#000; font-size:16px;font-weight:normal;height:52px;}
img.plus{vertical-align:middle;}
.updates{font-weight:normal;color:#777;text-align:left;padding:8px 5px 10px 5px;border-top:solid 1px #E5E5E5;margin:0px 10px;}
.updates:hover{color:#000;}
.timesince a{font-weight:normal;color:#000;}
.credit{margin:0;padding:0;text-align:right;display:block;height:18px;}
a.cre:hover, a.c{-moz-border-radius-topleft:8px;border-top-left-radius: 8px;
font-size:12px;font-weight:normal;
text-decoration:none;padding:3px 10px 3px 10px;color:#fff;background: #000;}
a.cre{-moz-border-radius-topleft:8px;border-top-left-radius: 8px;font-size:12px;font-weight:normal;text-decoration:none;padding:3px 10px 3px 10px;color:#fff;background:none;}
</style>
<?php
}
...
class Google_Plus_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !$gson = wp_cache_get( 'widget-gplus-' . $this->number , 'widget' ) ) {
			$google_json_url = esc_url_raw( 'https://www.googleapis.com/plus/v1/people/'. $account .'?pp=1&key=AIzaSyCE08aKDMh4dXsfhjbcBtr6SvR0TDUadxc' );
		...
		echo "<div class='widget_google_plus'><div class='author-box'><a rel='author' class='plus_person' target='_blank' title='Add to circles' href='" . esc_url( "https://plus.google.com/{$account}" ) . "'><strong>" . $g_title . "</strong></a> on    <a href='" . esc_url( "https://plus.google.com/{$account}" ) . "'
		...
		if ( !$hideupdates ) {
			...
			if ( !$activities_json = wp_cache_get( 'gplus-' . $this->number , 'widget' ) ) {
				...
				$google_activities_url = esc_url_raw( 'https://www.googleapis.com/plus/v1/people/'. $account .'/activities/public?alt=json&maxResults='. $items .'&pp=1&key=AIzaSyCE08aKDMh4dXsfhjbcBtr6SvR0TDUadxc' );
			...
			if ( !$hidecredit ) {
				echo '<div class="credit"><a class="c" href="http://patrick.bloggles.info/2011/08/01/google-plus-wordpress/">Google+ Widgets</a></div>';
			} else {
				echo '<div class="credit"><a class="cre" href="http://patrick.bloggles.info/2011/08/01/google-plus-wordpress/">Google+ Widgets</a></div>';
			}
	...
	function form( $instance ) {
		...
		if ( $hidecredit )
			echo ' checked="checked"';
		echo ' /> ' . esc_html__('Hide Credit') . '</label></p>';
